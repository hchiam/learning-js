...
function getWordSquares(words: string[]): string[][] { // Ot(n^2 c) or Ot(n^2 c^2)
    if (words[0].length === 1) {
    ...
    const output = [];
    for (let word of words) { // Ot(n) -> Ot(n^2 c) or Ot(n^2 c^2)
...
function buildHashTable(words: string[]): object { // Ot(n c^2)
    const ht = {};
    for (let word of words) { // Ot(n)
        for (let i = 0; i < word.length; i++) { // Ot(c)
    ...
    // trie: object): void 
{
    if (count === choicesCount) {
        ...
        const candidateWords: string[] = getWordsWithPrefix(nextValidPrefix, words, ht); // Ot(1)
        for (let word of candidateWords) {
            choices.push(word);
            backtrack(count+1,
                      choicesCount, choices, words, output, ht);
            choices.pop();
        }
    }
}

function getNextValidPrefix(i: number, choicesSoFar: string[]): string { // Ot(n)
    let prefix = '';
    for (let word of choicesSoFar) { // Ot(n)
        prefix += word[i];
    }
    return prefix;
}
...
function getWordsWithPrefix(prefix: string, words: string[], ht: object): string[] { // Ot(1)
    if (!prefix) return [];
    // let pointer = trie;
