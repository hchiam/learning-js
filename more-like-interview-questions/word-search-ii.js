...
BCR: time O(c + L) space O(1)
ideas:
1) remove cells that don't appear in letters? (see 2) below:)
2) have a HT to track all the valid letters for cells?
    time O(c + L) space O(L) for creating HT
    +
    time O(c * L) for checking each word at each cell
3) trie of valid words? time O(wordCount * avgWordLen) space O(L)
...
4) then checking cells against trie based on the hint:
    time O(wordCount * avgWordLen) space O(L) for creating trie
    or rather
    time O(L) space O(L) for creating trie
    +
    time O(c * wordLength) for checking cells against trie,
    which is better than time O(c * L) for checking cells against HT?
...
var findWords = function (board, words) {
  ...
  const visitedCells = new Array(rows)
    .fill(null)
    .map((x) => new Array(cols).fill(null));
  ...
  function checkCellForWord(i, j, p) {
    // p for pointer
